...
// 匹配 LCK 相关赛事: LCK Spring/Summer/Cup/Rounds 等, 排除非韩国一队联赛
function isLckTournament(name) {
  if (!name) return false;
  const trimmed = name.trim();
  if (trimmed === 'LCK') return false;
  if (/^LCK\s*CL\b/i.test(trimmed)) return false; // Challengers League (二队)
  if (/^LCK\s*Academy/i.test(trimmed)) return false;
  if (/^LCK\s*Allstars/i.test(trimmed)) return false;
  return /^LCK\b/i.test(trimmed);
}

function extractLckTournamentNames(html) {
  const names = new Set();
  for (const match of String(html || '').matchAll(/\bLCK[^<'"\n\r]{0,80}/g)) {
    const candidate = match[0].split(/[<'"]/)[0].trim();
    if (isLckTournament(candidate)) names.add(candidate);
  }
  return [...names];
}
...
async function discoverTournamentsForSeason(season) {
  ...
  return extractLckTournamentNames(html);
}
...
async function main() {
  ...
  console.log('Step 1/3: 查询每个赛季的 LCK 相关赛事名...');
  const candidates = [];
  ...
  for (const season of SEASONS) {
    try {
      ...
      for (const name of names) {
        ...
        seenNames.add(name);
        candidates.push({ tournament: name, season, year: seasonToYear(season) });
      }
      console.log(`  ${season}: 发现 ${names.length} 个`);
    } catch (error) {
      console.warn(`  ${season}: 查询失败 ${error.message}`);
    }
    await sleep(300);
  }
  ...
  await writeFile(
    path.join(HISTORY_DIR, 'lck_tournament_candidates.json'),
    JSON.stringify({
  ...
  console.log(`  汇总: ${candidates.length} 个赛事 -> lck_tournament_candidates.json`);
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    try {
      ...
      if (!teams.length) {
        emptyCount += 1;
        console.warn(`  [${i + 1}/${candidates.length}] ${c.tournament}: 空（跳过）`);
      } else {
        okCount += 1;
  ...
  await writeCsv(
    ...
    auditRows,
    ['gol_team_id', 'gol_team_name', 'team_key_raw', 'tournament', 'season', 'year', 'games_in_tournament', 'source_url'],
  );
  await writeCsv(
  ...
  console.log('');
  console.log('文件:');
  console.log(`  ${path.join(HISTORY_DIR, 'lck_tournament_candidates.json')}`);
  console.log(`  ${path.join(DATA_DIR, 'team_alias_audit.csv')}`);
  console.log(`  ${path.join(DATA_DIR, 'team_alias_audit_distinct.csv')}`);
}

main().catch((error) => {
  console.error(`LCK discover-teams 失败: ${error.message}`);
  process.exitCode = 1;
});
