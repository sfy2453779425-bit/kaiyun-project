...
const ODDS_FILE = path.join(HISTORY_DIR, 'oddsportal_lck_match_odds.csv');
...
function tournamentUrl(year) {
  const suffix = String(year) === '2026' ? 'league-of-legends-lck' : `league-of-legends-lck-${year}`;
...
async function fetchArchivePage(archiveUrl, referer, page = 1) {
  const res = await fetch(ajaxUrl(archiveUrl, page), {
    headers: {
      'user-agent': 'Mozilla/5.0 LCK odds history collector',
...
function markdownReport({ oddsRows, joinedRows, unmatchedCount, selectionSummaries, favoriteSummaries, edgeRows }) {
  const lines = [];
  lines.push('# OddsPortal LCK 历史赔率 vs 模型');
  lines.push('');
...
async function main() {
  ...
  // --reuse: 复用本地已抓的赔率, 只重新对账(模型改了但赔率没变时用, 避免被 OddsPortal 限流)
  const reuse = process.argv.includes('--reuse');
  let oddsRows;
  if (reuse) {
    oddsRows = await readCsv(ODDS_FILE);
    console.log(`复用本地赔率: ${oddsRows.length} 场 (${ODDS_FILE})`);
  } else {
    oddsRows = await collectOdds(years);
...
main().catch((error) => {
  console.error(`OddsPortal LCK historical odds comparison failed: ${error.stack || error.message}`);
  process.exitCode = 1;
});
