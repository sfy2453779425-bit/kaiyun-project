...
export const DATA_DIR = path.join(ROOT, 'lpl', 'data');
  ...
  for (let attempt = 1; attempt <= tries; attempt += 1) {
    try {
      const res = await fetch(url, {
        ...options,
        headers: {
          'user-agent': 'Mozilla/5.0 LPL market model',
          ...(options.headers || {}),
        },
      });
...
export function golUrl(kind, tournament = DEFAULT_TOURNAMENT) {
  if (!tournament) throw new Error('golUrl 需要 tournament 参数（LPL 项目不允许默认值）');
...
// Phase 1.2 输出：基于 team_alias_audit.csv 整理。
// 规则:把同一支 LPL 队伍的所有别名(英文全名/中文/缩写/gol.gg slug)map 到稳定 3 字母 key。
// 备注:
//   - Victory Five (2022) 与 Ninjas in Pyjamas (2023+) 是同一家俱乐部改名后产物,
//     但赛季阵容不同、gol.gg team_id 也不连续,backtest 上不应跨年合并历史。
//     故保留 V5 / NIP 两个独立 key,不做别名合并。
//   - ThunderTalk Gaming 的简称就是 TT,gol.gg 在 2022-2025 都用 'TT',2026 起改用全称 'ThunderTalk Gaming',
//     是同一队不同显示名,合并到 TT。
//   - Oh My God 与 OMG 同队,合并到 OMG。
//   - EDward Gaming 在 2026 出现新拼写(D 大写),与 Edward Gaming 同队,合并到 EDG。
const TEAM_ALIASES = {
  anyoneslegend: 'AL',
  anyonelegend: 'AL', // 防止 gol.gg 把 "Anyone's Legend" 渲染成无 s 的 "Anyone Legend" 时拆队
  al: 'AL',
  bilibiligaming: 'BLG',
  blg: 'BLG',
  edwardgaming: 'EDG',
...
export function teamKey(name) {
  ...
  const compact = raw
    ...
    .replace(/\.|\s|-/g, '')
    .replace(/[^a-z0-9]/g, '');
  return TEAM_ALIASES[compact] || raw.toUpperCase();
}

export function displayTeam(key, fallback = '') {
  return TEAM_DISPLAY[key] || fallback || key;
}
