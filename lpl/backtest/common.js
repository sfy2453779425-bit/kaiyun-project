...
export const BACKTEST_DIR = path.join(DATA_DIR, 'backtest');

// Phase 4.3: 2024 = in_sample (跟 Phase 2 baseline 同源,会被调参偏置),
// 2025 = out_of_sample (纯验证)。
export const SPLIT_IN_SAMPLE = 'in_sample_2024';
export const SPLIT_OUT_SAMPLE = 'out_of_sample_2025';

export function splitForDate(dateStr) {
  const yr = String(dateStr || '').slice(0, 4);
  if (yr === '2024') return SPLIT_IN_SAMPLE;
  if (yr === '2025') return SPLIT_OUT_SAMPLE;
  return '';
}
...
function emptyTeamAccumulator(teamId, teamName) {
  return {
    ...
    first_blood_denom: 0,
    first_turrets: 0,
    first_turret_denom: 0,
    first_heralds: 0,
    first_herald_denom: 0,
    match_ids: new Set(),
    match_wins: 0,
    two_zero_wins: 0,
    zero_two_losses: 0,
...
function addMapSide(groups, map, side) {
  ...
  // LPL FB/FT 缺失修复 (Phase 2/3 决定): 分母只算字段非空的 map。
  const fbKey = teamKey(map.first_blood_team_id || map.first_blood_team);
  if (fbKey) {
    acc.first_blood_denom += 1;
    if (fbKey === sideKey) acc.first_bloods += 1;
  }
  const ftKey = teamKey(map.first_turret_team_id || map.first_turret_team);
  if (ftKey) {
    acc.first_turret_denom += 1;
    if (ftKey === sideKey) acc.first_turrets += 1;
  }
  const fhKey = teamKey(map.first_herald_team_id || map.first_herald_team);
  if (fhKey) {
    acc.first_herald_denom += 1;
    if (fhKey === sideKey) acc.first_heralds += 1;
  }
...
export function buildSnapshotSummary(maps, cutoffDate) {
  ...
  return [...teams.values()].map((team) => {
    ...
    return {
      ...
      first_blood_rate: team.first_blood_denom ? team.first_bloods / team.first_blood_denom : 0,
      first_turret_rate: team.first_turret_denom ? team.first_turrets / team.first_turret_denom : 0,
      dragon_control_rate: dragonDenom ? team.dragons / dragonDenom : '',
      herald_control_rate: team.first_herald_denom ? team.first_heralds / team.first_herald_denom : 0,
...
export async function writeBacktestCsv(fileName, rows, columns = unionColumns(rows)) {
  ...
  await writeCsv(path.join(BACKTEST_DIR, fileName), rows, columns);
}
