...
const TOURNAMENT = argValue('tournament', process.env.LPL_TOURNAMENT || '');
const MAX_MATCHES = argNumber('max-matches', 0);
const SKIP_GAMES = hasFlag('no-games');
const OUTPUT_DIR = argValue('output-dir', '');

if (!TOURNAMENT) {
  console.error('错误: 必须传入 --tournament="赛事完整名称"（LPL 赛事名变化频繁,不允许默认值）');
  console.error('例: node lpl/collect-lpl.js --tournament="LPL 2026 Split 1"');
  process.exit(2);
}
...
function buildTeamSummary(mapRows, matchRows, golTeamRows) {
  ...
  return [...teams].map(([teamId, team]) => {
    ...
    for (const map of maps) {
      ...
      if (map.first_blood_team_id) {
        fbDenom += 1;
        if (map.first_blood_team_id === teamId) fb += 1;
      }
      if (map.first_turret_team_id) {
        ftDenom += 1;
        if (map.first_turret_team_id === teamId) ft += 1;
      }
    ...
    return {
      ...
      first_blood_rate: useMapStats && fbDenom ? pct(fb, fbDenom).toFixed(3) : parsePercent(gol.fbpct).toFixed(3),
      first_turret_rate: useMapStats && ftDenom ? pct(ft, ftDenom).toFixed(3) : parsePercent(gol.ftpct).toFixed(3),
...
async function main() {
  const outputDir = OUTPUT_DIR ? path.resolve(OUTPUT_DIR) : DATA_DIR;
  const rawDir = OUTPUT_DIR ? path.join(outputDir, 'raw') : RAW_DIR;
  await mkdir(outputDir, { recursive: true });
  await mkdir(rawDir, { recursive: true });
  await mkdir(path.join(outputDir, 'public_sources'), { recursive: true });

  console.log(`采集 LPL 公开数据: ${TOURNAMENT}`);
  if (OUTPUT_DIR) console.log(`输出目录(自定义): ${outputDir}`);
  const urls = {
  ...
  await writeFile(path.join(rawDir, 'gol_matchlist.html'), matchHtml, 'utf8');
  await writeFile(path.join(rawDir, 'gol_teams.html'), teamHtml, 'utf8');
  await writeFile(path.join(rawDir, 'gol_players.html'), playerHtml, 'utf8');
  await writeFile(path.join(rawDir, 'gol_champions.html'), championHtml, 'utf8');
  ...
  await writeCsv(path.join(outputDir, 'public_sources', 'gol_matchlist_stats.csv'), matchRows, unionColumns(matchRows));
  await writeCsv(path.join(outputDir, 'public_sources', 'gol_team_stats.csv'), teamTable.rows, unionColumns(teamTable.rows));
  await writeCsv(path.join(outputDir, 'public_sources', 'gol_player_stats.csv'), playerTable.rows, unionColumns(playerTable.rows));
  await writeCsv(path.join(outputDir, 'public_sources', 'gol_champion_stats.csv'), championTable.rows, unionColumns(championTable.rows));

  const { mapRows, playerRows } = SKIP_GAMES
    ? {
        mapRows: await readCsvIfExists(path.join(outputDir, 'lpl_map_details.csv')),
        playerRows: await readCsvIfExists(path.join(outputDir, 'lpl_player_map_details.csv')),
      }
    : await collectGameDetails(matchRows);

  const recentForm = buildRecentForm(matchRows).map((row) => ({ ...row, tournament: TOURNAMENT }));
  const teamSummary = buildTeamSummary(mapRows, matchRows, teamTable.rows);
  const teams = teamSummary.map((row) => ({ team_id: row.team_id, team_name: row.team, tournament: TOURNAMENT }));

  await writeCsv(path.join(outputDir, 'lpl_matches.csv'), matchRows, [
  ...
  await writeCsv(path.join(outputDir, 'lpl_teams.csv'), teams, ['team_id', 'team_name', 'tournament']);
  await writeCsv(path.join(outputDir, 'lpl_recent_form.csv'), recentForm, [
    'team_id', 'team_name', 'tournament', 'played', 'wins', 'losses', 'maps_won', 'maps_lost',
    'win_rate', 'recent_5',
  ]);
  await writeCsv(path.join(outputDir, 'lpl_map_details.csv'), mapRows, unionColumns(mapRows));
  await writeCsv(path.join(outputDir, 'lpl_player_map_details.csv'), playerRows, unionColumns(playerRows));
  await writeCsv(path.join(outputDir, 'lpl_team_detail_summary.csv'), teamSummary, unionColumns(teamSummary));

  await writeFile(path.join(outputDir, 'latest-summary.json'), JSON.stringify({
  ...
  await writeFile(path.join(outputDir, 'public_sources', 'source_notes.txt'), [
    'LPL public source notes',
    '',
    `Tournament: ${TOURNAMENT}`,
  ...
  console.log(path.join(outputDir, 'lpl_matches.csv'));
}

main().catch((error) => {
  console.error(`LPL数据采集失败: ${error.message}`);
  process.exitCode = 1;
});
